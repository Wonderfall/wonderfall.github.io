...
- Even if your private key is encrypted, malicious applications could register your keystrokes.
...
Restart the `sshd` service and try to connect to your server using your key handles (by passing `-i ~/.ssh/id_ed25519_sk` to `ssh` for instance). If that works for you (your FIDO2 security key should be needed to derive the real secret), feel free to remove your previous keys from `.ssh/authorized_keys` on your server.
